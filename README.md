...
- `1. 문서 넣기`: PDF·DOCX·이미지에서 텍스트를 꺼내고 chunk, BM25, Chroma,
  RRF, 로컬 BGE 재정렬을 단계별로 확인합니다.
- `2. 질문하기`: LangGraph가 실제로 만든 답변을 읽고, 답변에 연결된 원문 구간과
  관리자용 검색 점수를 같은 화면에서 확인합니다.

답변 결과 안의 `답변`, `근거 검증`, `검색 기록 (관리자)`는 별도 목업 웹사이트가
아닙니다. 같은 Python RAG 실행 결과를 사용합니다. 일반 사용자 화면에는 검색
점수를 숨기고, 관리자 화면에서만 BM25·벡터·RRF·로컬 BGE 점수를 보여줍니다.

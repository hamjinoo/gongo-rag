...
| 서비스 | Streamlit RAG 안에 DocLens Trace 답변·근거 검증·검색 기록 통합 | Docker와 재현 가능한 실행 환경 |
...
- `프로젝트 한눈에`: 해결하려는 문제, 전체 아키텍처, 핵심 성과를 먼저 설명합니다.
- `RAG 데모`: 실제 답변, 실행된 LangGraph 단계, 검색 순위 변화와 원문 근거를
  한 흐름으로 확인합니다.
- `문서 실험실`: PDF·DOCX·이미지에서 텍스트를 꺼내고 chunk, BM25, Chroma,
  RRF, 로컬 BGE 재정렬을 단계별로 비교합니다.
- `평가 결과`: dev/test 성능, BGE 선택 근거와 자동 평가의 실패 사례를 공개합니다.

답변 결과 안의 `답변`, `실행 과정`, `근거 검증`, `검색 상세`는 별도 목업
웹사이트가 아닙니다. 같은 Python RAG 실행 결과를 사용합니다. `실행 과정`에서는
BM25·벡터·RRF·로컬 BGE 순위가 어떻게 바뀌었는지 보여주고, `검색 상세`에서는
실제 원점수까지 확인할 수 있습니다.

브라우저의 `문서 실험실` 탭에서 일반 PDF, 스캔 PDF, DOCX, 이미지를
...
```text
gongo-rag/
├── app.py                 # 업로드·추출·질문을 합친 Streamlit 앱
...
├── notes/                 # 관찰 기록
├── src/                   # 추출·검색·reranker·LangGraph·DocLens Trace UI·평가
└── tests/                 # 핵심 로직 자가 검증
```
...
2. 업로드 문서의 저장·재색인 흐름을 한 번에 연결하기

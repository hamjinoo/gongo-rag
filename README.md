...
| 검색 | Kiwi BM25 + multilingual E5 + 로컬 Chroma 의미 검색 | 두 검색 순위를 RRF로 결합 |
...
flowchart LR
    ...
    D --> F["각 검색기의 Top-k + 출처"]
    E --> F
    F --> U["Streamlit에서 결과 비교"]
    F -. "다음: RRF · reranker" .-> G["LLM 답변 + 인용"]
...
├── notes/                 # 관찰 기록
├── src/                   # 문서 추출·chunking·BM25·Chroma·답변·평가
└── tests/                 # 핵심 로직 자가 검증
```
...
1. Kiwi BM25 + E5/Chroma 검색을 RRF로 결합하기
2. CrossEncoder reranking 전후 성능·지연 비교하기
3. LangGraph 재검색·거절 흐름 구현하기
4. Ragas·수동 검토를 포함한 평가표 작성하기

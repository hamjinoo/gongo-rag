...
`RAG 실행`이 끝나면 입력 폼 대신 답변·직접 근거·검증 결과가 화면 맨 위에
나타납니다. 그 아래에서 실제 후보 수가 `BM25+Embedding → RRF → BGE → 인용 →
검증`으로 줄어드는 과정, 인용 Chunk의 단계별 순위 변화, 문서 준비·검색·생성·검증
시간을 먼저 보여줍니다. 네 단계 전체 Top-k와 Chunk ID·원점수는 실행 추적 펼침

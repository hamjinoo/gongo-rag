...
처음부터 각 구성요소의 필요성을 공고문 데이터에서 발견해 하나씩 추가한 프로젝트는
아닙니다. `BM25 + Embedding + RRF + reranker`라는 일반적인 Hybrid RAG 구조를
**검증할 초기 가설**로 먼저 구현했습니다. 이후 같은 문서·Chunk·질문으로 구성요소를
비교하며 실제로 필요한지 확인하고 있습니다.

현재 결과는 “기술을 많이 붙일수록 항상 좋아진다”는 가설을 지지하지 않습니다.

| 구성요소 | 처음 기대한 역할 | 현재 데이터에서 확인한 점 |
|---|---|---|
| BM25 | 날짜·사업명처럼 정확한 단어 검색 | 가장 빠르고 test Hit@1 0.80으로 강한 기준선 |
| Embedding | 표현이 다른 같은 뜻 보완 | 일부 문항은 보완했지만 전체 성능은 BM25보다 낮음 |
| RRF | 두 검색기의 점수 단위 없이 후보 결합 | 후보 결합은 가능하지만 BM25보다 항상 좋아지지는 않음 |
| BGE reranker | 후보를 질문과 함께 읽어 최종 순위 개선 | 최종 파이프라인은 BM25의 dev MRR 0.827 대비 0.925지만 CPU 평균 약 4.20초 필요 |

따라서 현재 파이프라인은 최종 정답이 아니라 **v0 품질 우선 기준선**입니다. 마지막
검색 구조 결정 전 `BM25 Top-7 → BGE`와 `RRF Top-7 → BGE`를 dev에서 비교합니다.
결과가 같다면 Embedding·RRF를 기본 실행에서 제거하고, Hybrid가 실제 문항을
살린다면 그 사례를 근거로 유지합니다.
...
| 검색 | Kiwi BM25 + E5/Chroma + chunk ID 기반 RRF 통합 검색 | BM25→BGE와 RRF→BGE ablation |
| 재정렬 | 로컬 BGE + 후보 7개 v0 설정 | 구성요소별 기여 확인 후 최종 결정 |
...
현재 v0 구조는 `LangChain → BM25/Chroma → RRF → reranker → LangGraph`입니다.
구조를 그대로 유지하는 것이 목표가 아니라, 같은 dev 평가셋에서 구성요소를 뺐을 때의
성능과 지연을 비교해 더 단순한 구조로 같은 품질을 낼 수 있는지 확인하는 것이
목표입니다.
...
`RAG 실행`이 끝나면 답변·직접 근거·검증 결과를 하나의 결과 카드로 화면 맨 위에
보여줍니다. 그 아래에서 `문서 처리 → BM25 → Embedding → RRF → BGE → 답변·검증`
6단계로 후보가 줄어드는 과정, 인용 Chunk의 순위 변화, 단계별 실행 시간 막대를
보여줍니다. 네 단계 전체 Top-k와 Chunk ID·원점수는 실행 추적 펼침
영역에서만 확인합니다. 후보 축소 파이프라인의 각 박스에 마우스를 올리면 해당
단계의 실제 Top-3 Chunk가 잠깐 보이고, 클릭하면 열린 상태로 고정됩니다. 입력
문서·문서별 Chunk 수·검색 출처·페이지·순위·점수를 긴 스크롤 없이 확인할 수
있습니다. 화면의 데이터는 별도 목업이 아니라 같은 Python RAG 실행에서 기록한
실제 결과입니다.
...
- 검색 구조는 dev에서 `BM25→BGE`와 `RRF→BGE`를 비교한 뒤 최종 결정합니다.
...
1. BM25→BGE와 RRF→BGE의 마지막 ablation 실행하기

...
| 영역 | 현재 구현 | 다음 단계 |
|---|---|---|
| 문서 처리 | PDF·DOCX·이미지 추출, 고정 길이·문단 우선 chunking, metadata 미리보기 | LangChain Document 변환 후 색인 저장 |
| 검색 | 구조화 chunk를 검색하는 Kiwi 한국어 BM25, 기존 SBERT 실험 코드 | ChromaDB 의미 검색 + RRF |
...
업로드한 chunk를 embedding하고 ChromaDB에 저장하는 일은 다음 단계입니다.
...
1. 현재 chunk를 LangChain Document로 변환하기
2. embedding을 만들고 ChromaDB에 저장·검색하기
3. Kiwi BM25 + dense 검색을 RRF로 결합하기
4. CrossEncoder reranking 전후 성능·지연 비교하기
5. LangGraph 재검색·거절 흐름 구현하기
6. Ragas·수동 검토를 포함한 평가표 작성하기
7. FastAPI·Docker와 재현 가능한 실행 환경 제공하기

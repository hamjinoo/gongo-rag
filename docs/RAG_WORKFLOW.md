...
| 8. v0 검색 설정 기록 | ✅ | 로컬 BGE·후보 7개로 test 한 번 완료 |
| 8-1. 검색 구조 ablation | ⬜ | BM25→BGE와 RRF→BGE를 dev에서 비교 |
| 9. LangGraph 답변 | ✅ | 근거 판단·재검색 1회·인용·거절 흐름 |
| 10. Ragas 답변 평가 | ⬜ | 근거와 답변이 맞는지 측정 |
| 11. 서비스·포트폴리오 | ⬜ | FastAPI·Docker·실행 안내 |

현재 **검색 기능 구현과 v0 test 기록은 완료**됐습니다. 다만 Embedding·RRF가
최종 BGE 결과에 실제로 필요한지 확인하는 ablation이 남아 있으므로 최종 구조라고
단정하지 않습니다.
...
검증을 합친 결과 카드, 6단계 후보 축소, 최종 근거의 순위 변화와 실행 시간 비교
막대만 둡니다. 전체 Top-k·전체 Chunk·원점수는 추적이 필요할 때만 펼칩니다. 따라서
서비스 결과와 엔지니어링 관측 정보를 모두 보존하면서도 디버깅 화면이 답변보다 먼저
보이지 않습니다. 후보 축소 파이프라인은 단계 박스를 가리키면 Top-3 Chunk를
미리 보여주고, 클릭하면 팝오버를 고정합니다. BM25·Embedding에는 출처·페이지와
원점수를, RRF에는 두 검색 순위를, BGE에는 RRF 대비 순위 변화를 보여줘 각 단계가
무엇을 바꿨는지 바로 비교할 수 있습니다.
...
### 5-3. 마지막 검색 구조 ablation

지금까지는 RRF 후보를 BGE가 재정렬한 결과와 BM25·Chroma·RRF 단독 결과를
비교했습니다. 아직 아래 두 전체 경로는 직접 비교하지 않았습니다.

```text
A. BM25 Top-7 → BGE
B. BM25 + Embedding → RRF Top-7 → BGE
```

같은 dev normal 20문항으로 Hit@1·3·5, MRR, 평균·p95 지연과 질문별 순위 변화를
비교합니다. A와 B의 품질이 같다면 더 단순한 A를 기본값으로 선택합니다. B가 실제
문항을 살린다면 해당 문항을 근거로 Embedding·RRF를 유지합니다. 이미 확인한 test
결과에 맞춰 설정을 조정하지 않습니다.

완료 기준:

- [ ] BM25→BGE 결과
- [ ] RRF→BGE와 동일 조건 비교
- [ ] Embedding이 살린 사례와 Hybrid가 해친 사례 기록
- [ ] 품질·지연을 근거로 최종 구조 선택

---

### 5-4. Ragas와 사람 검토로 답변 평가
...
```text
1. BM25→BGE vs RRF→BGE ablation
2. Ragas + 사람 답변 평가
3. FastAPI
4. Docker
5. 최종 README·포트폴리오 정리
```

### 바로 다음 요청 예시

```text
같은 dev normal 20문항에서 BM25 Top-7→BGE와 RRF Top-7→BGE를 비교해줘.
Hit@1·3·5, MRR, 평균·p95 지연과 질문별 정답 순위 변화를 기록하고, Embedding·RRF가
실제로 살린 질문과 오히려 해친 질문을 분리해줘. test는 다시 튜닝에 사용하지 말고
결과에 따라 더 단순한 최종 검색 구조를 선택해줘.
```

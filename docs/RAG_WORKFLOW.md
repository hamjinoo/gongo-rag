...
4. Cohere 실제 비교 — API 키가 없어 보류, 로컬 BGE 선택
5. BGE + 후보 7개로 검색 설정 고정 — 완료
6. test normal 10문항 검색 평가 한 번 — 완료
...
```text
잠근 검색 파이프라인을 LangGraph에 연결해줘. 근거가 충분하면 출처와 함께 답하고,
부족하면 질문을 한 번 고쳐 재검색한 뒤 그래도 없으면 정보 없음으로 거절해줘.
성공·재검색·거절 세 경로를 테스트하고 12살도 이해할 문서도 업데이트해줘.
```
...
> 같은 평가 인터페이스에 연결했지만 키가 없어 실제 비교는 보류했습니다. 로컬
> BGE + 후보 7개로 test를 한 번 실행해 Hit@1 0.80, MRR 0.85를 확인했고 검색
> 설정을 잠갔습니다. 이제 LangGraph로 재검색·인용·거절 흐름을 연결하고 Ragas와
> 사람 검토로 최종 답변을 평가할 계획입니다.

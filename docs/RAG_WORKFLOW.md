...
| 8. 검색 설정 확정 | 🔄 | 작은 모델 비교 완료, Cohere 결정 후 test 한 번 |
...
3. Cohere Rerank를 비교할 필요가 있는지 결정
4. 비교한다면 품질·속도·비용·문서 외부 전송 여부를 함께 기록
5. 검색 설정을 고정한 뒤 test 검색 평가를 한 번만 실행
...
```text
Cohere Rerank를 BGE와 실제로 비교할 가치가 있는지 비용, 문서 외부 전송,
포트폴리오 필요성을 기준으로 판단해줘. 비교한다면 후보 7개와 dev 질문만
사용하고 test는 실행하지 마.
```
...
> 약 9.8배 빨랐지만 Hit@1이 0.70으로 떨어져 BGE를 유지했습니다. 이제 Cohere
> 비교 여부를 결정하고 검색 설정을 고정한 뒤, LangGraph로 재검색·인용·거절

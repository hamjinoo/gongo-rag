...
| 8. 검색 설정 확정 | 🔄 | Cohere 연결 완료, API dev 비교 후 test 한 번 |
...
3. Cohere Rerank 비교 가치 판단과 API adapter — 완료
4. API 키가 준비되면 같은 dev 20문항으로 실제 비교
5. 품질·속도·search unit·문서 외부 전송 여부를 함께 기록
6. 검색 설정을 고정한 뒤 test 검색 평가를 한 번만 실행
...
- [x] 작은 로컬 모델 비교
- [x] Cohere 비교 결정과 안전한 API adapter
- [ ] Cohere dev 20문항 실제 비교
- [ ] 최종 검색 설정 고정
- [ ] test 검색 평가 한 번
...
```text
1. API 키로 Cohere dev 비교
2. BGE 또는 Cohere 선택과 검색 설정 고정
3. test 검색 평가 한 번
4. LangGraph 답변·재검색·거절
5. Ragas + 사람 답변 평가
6. FastAPI
7. Docker
8. 최종 README·포트폴리오 정리
```

### 바로 다음 요청 예시

```text
COHERE_API_KEY를 환경 변수에 설정했어. 후보 7개와 dev normal 20문항만 사용해
BGE와 Cohere rerank-v4.0-pro의 품질, API 지연, search unit을 비교해줘.
원문은 공개 공고문만 전송하고 test는 실행하지 마.
```
...
> 약 9.8배 빨랐지만 Hit@1이 0.70으로 떨어져 BGE를 유지했습니다. Cohere API도
> 같은 평가 인터페이스에 연결했으며, 키가 준비되면 공개 공고문 dev 20문항만
> 비교합니다. 그 결과로 검색 설정을 고정한 뒤 LangGraph로 재검색·인용·거절
> 흐름을 연결하고 Ragas와 사람 검토로 최종 답변을 평가할 계획입니다.

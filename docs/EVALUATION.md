...
### 11-5. Chroma 단독은 정답 두 개를 후보 10개에 넣지 못했습니다
...
### 11-6. RRF의 역할은 최종 1위보다 후보 회수였습니다
...
- MiniLM은 속도 우선 선택지
- Cohere 비교 adapter는 구현, 실제 dev 수치는 API 키 대기
...
> 0.70이면서 0.6ms였고, RRF는 Hit@10 1.0으로 후보 회수 역할을 확인했습니다.
> no-answer 문항은 검색 순위와 섞지 않고 이후 답변 거절 평가에 사용하며, 설정을
> 고른 뒤 test는 한 번만 실행할 계획입니다.
...
후보 7개가 dev Hit@1 0.85를 기록하며 평균 지연을 약 4.20초로 줄였지만 여전히
느립니다. 작은 MiniLM은 약 9.8배 빨랐지만 Hit@1이 0.70으로 떨어져 기본값으로
채택하지 않았습니다. GPU·최적화 또는 Cohere는 같은 dev 질문으로 비교해야 합니다.
Cohere 경로는 이미 같은 평가기에 연결했지만 아직 API 키가 없어 실제 수치는 없습니다.
...
API 키가 준비되면 공개 공고문과 dev 20문항으로 Cohere의 품질·지연·search unit을
한 번 측정합니다. BGE와 비교해 검색 설정을 고정하고 test를 한 번 실행한 뒤

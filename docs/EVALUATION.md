...
- MiniLM은 속도 우선 선택지
- Cohere adapter는 유지하되 실제 비교는 API 키 부재로 보류
...
하락했고 Cohere는 API 키가 없어 실제 비교를 보류했습니다. 운영 최적화는 별도
...
검색 설정, test 확인, LangGraph 답변 흐름 연결까지 끝났습니다. 다음은 normal
답변의 관련성·근거 충실성과 no-answer 거절을 Ragas와 사람 검토로 평가합니다.

...
- [x] 외부 reranker API 생략, 로컬 BGE로 범위 고정
...
있습니다. 외부 reranker API는 생략하고 로컬 BGE·후보 7개를 최종 검색 설정으로
선택했습니다. 이 설정으로 test normal 10문항을
...
리랭커는 검색 결과를 다시 채점하는 심사위원입니다. 이 프로젝트에서는 인터넷에
답안지를 보내는 심사위원을 추가하지 않고, 내 컴퓨터에서 일하는 BGE만 씁니다.

- 별도의 reranker API 키가 필요 없습니다.
- 질문과 문서 후보가 재정렬 단계에서 외부로 나가지 않습니다.
- 평가 때 쓴 모델 revision과 실행 조건을 직접 고정할 수 있습니다.
- 도구 비교보다 검색 품질과 실패 분석에 학습 시간을 집중할 수 있습니다.

운영 환경에서 속도가 문제가 되면 새 dev/holdout 질문을 만든 뒤 작은 로컬 모델,
양자화, ONNX/OpenVINO, GPU 적용을 각각 하나씩 비교합니다.
...
> 선택지로만 남겼습니다. 외부 reranker API는 생략하고 로컬 BGE·후보 7개로
> test를 한 번 실행했습니다. test Hit@1은 0.80, MRR은 0.85였으며 이 결과로
> 다시 튜닝하지

...
다음은 후보 수, 최적화 모델과 Cohere를 dev에서 비교해 약 6.28초인 CPU 지연을
줄이는 작업입니다.
...
> 실패도 확인했습니다. 이후 dev 20문항에서 비교한 결과 reranker는 Hit@1 0.80,
> MRR 0.90으로 가장 높았지만 CPU 평균 latency가 약 6.28초였습니다. 따라서 다음은
> 후보 수와 더 작은 모델, Cohere를 같은 dev 질문으로 비교하는 단계입니다.
...
dev 비교에서 확인한 reranker의 약 6.28초 평균 latency를 줄이기 위해 후보 수,
더 작은 로컬 모델, Cohere를 비교합니다. 설정을 잠근 뒤 test를 한 번만 실행하고,

...
후보 수 dev 비교 후 현재 기본 설정에서는 RRF 상위 7개를 요청합니다.

```python
candidates = hybrid_retriever.search(query, k=7)
```
...
4. 그래서 고정 질문으로 Hit@1, MRR, nDCG를 비교했고, 한 사례가 아닌 전체 경향을 확인했습니다.
5. 최종 답변 단계에서는 여러 근거를 함께 읽고 대상과 제외 조건을 구분해야 합니다.

이 사례는 실패를 숨긴 것이 아니라, 고정 평가가 왜 필요한지 보여주는 근거입니다.
...
같은 dev 질문으로 네 검색 방식을 비교하는 평가까지 완료했습니다.
...
실제 결과와 해석은
[7번 작업: 같은 시험지로 검색기 성적 비교하기](EVALUATION.md)에 정리했습니다.
...
로컬 BGE는 우리 집 컴퓨터에서 일하는 심사위원입니다. Cohere는 답안지를 외부의
전문 심사위원에게 보내 점수를 받아오는 방식입니다.

| 항목 | 로컬 CrossEncoder | Cohere Rerank |
|---|---|---|
| API 키 | 불필요 | 필요 |
| 문서 외부 전송 | 없음 | 질문과 후보 본문을 외부 API로 전송 |
| 초기 준비 | 모델 다운로드·메모리 필요 | SDK와 API 설정 |
| 비용 | 장비·운영 비용 | search unit 또는 배포 인스턴스 비용 |
| 운영 | 직접 최적화 | 관리형 서비스 |
| 모델 변경 통제 | 직접 고정 가능 | 제공 모델 정책에 따름 |

현재 기본 API 모델은 품질 우선 다국어 모델인 `rerank-v4.0-pro`입니다.
`rerank-v4.0-fast`도 허용하지만 한 실험에서 모델을 하나만 바꿉니다.

```text
같은 RRF 후보 7개
├─ local  → BGE가 컴퓨터 안에서 점수 계산
└─ cohere → 질문 + 후보 본문을 API로 전송해 점수 수신
```

둘 다 `PairScorer.score_pairs(query, passages)`를 구현하므로 RRF, 평가 metric,
골든셋 코드는 바꾸지 않습니다. Cohere 응답은 점수가 높은 순서로 돌아오므로
`index`를 사용해 원래 후보 순서에 맞는 점수 배열로 복원합니다.

다음 오류는 결과를 조용히 믿지 않고 즉시 실패시킵니다.

- API 키 없음, 인증·네트워크·rate limit 오류
- 후보보다 적거나 많은 결과
- 중복·범위 밖 document index
- 숫자가 아닌 점수, NaN, 무한대

### 비용과 rate limit

Cohere는 Rerank 사용량을 search unit으로 반환합니다. 공식 가격 정의에서 한 번의
질문으로 최대 100개 문서를 정렬하면 한 search unit이며, 긴 문서는 내부에서
분할돼 문서 수가 늘어날 수 있습니다. 이 프로젝트는 질문마다 후보 7개를 보내고
응답의 `billed_search_units`를 합산해 JSON에 기록합니다.

무료 평가 키는 Rerank 10 requests/minute 제한이 있으므로 품질 실험은 가능하지만
그 latency를 production 성능으로 말하면 안 됩니다. 실제 운영 비용은 실행 시점의
공식 가격과 응답 usage를 다시 확인합니다.

### 개인정보와 공고문 전송

공개된 정부 공고문으로 dev 비교하는 것은 가능하지만, 사내 문서·개인정보·비공개
자료는 승인 없이 public API에 보내지 않습니다. Cohere Platform에는 API data
opt-out이 있지만, zero data retention은 별도 승인을 받은 enterprise 고객용
...
> BM25와 multilingual E5/Chroma 결과를 RRF로 합친 뒤, 상위 후보만
...
작은 값을 선택합니다. 실제 dev에서는 후보 5개가 RRF 6·7위 정답을 잃었고,
7개는 모두 포함하면서 후보 10개보다 평균 지연이 33.1% 낮아 7개를 골랐습니다.
...
- [Cohere Rerank 모델](https://docs.cohere.com/docs/rerank)
- [Cohere Rerank v2 API](https://docs.cohere.com/v2/reference/rerank)
- [Cohere API key와 rate limit](https://docs.cohere.com/v2/docs/rate-limits)

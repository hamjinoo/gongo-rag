...
지금은 어느 검색기가 “항상 더 좋다”고 결론 내리지 않습니다. 실패 유형을
모으는 것이 다음 RRF 단계의 근거가 됩니다.
...
- 아직 BM25와 Chroma 결과를 RRF로 합치지 않습니다.
...
- [ ] BM25와 Chroma 순위를 RRF로 결합
- [ ] reranker 추가

## 다음 작업

다음에는 새 검색기를 만들지 않고, 지금 있는 두 검색 결과를 합칩니다.
...
더하지 않습니다. 각 검색기의 **순위**를 이용하는 RRF를 구현합니다.
...
따라서 다음 RRF 단계는 BM25와 Chroma의 서로 다른 객체 형식을 다시
파싱하지 않고 공통 `chunk.id`로 합칠 수 있습니다.

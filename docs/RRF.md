...
- RRF 후보는 CrossEncoder reranker와 LangGraph 답변·재검색·거절 흐름에 연결했습니다.
...
0.85에서 0.70으로 떨어져 BGE를 기본값으로 유지했습니다. 외부 reranker API는
생략했습니다. 로컬 BGE와 후보 7개로 test를 한 번 실행해 검색 설정을 잠갔고
LangGraph 답변·재검색·거절 흐름에도
연결했습니다. 다음은 실제 답변과 no-answer 거절을 Ragas와 사람 검토로 평가합니다.

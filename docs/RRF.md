...
- dev 20문항으로 비교했지만 test split은 아직 실행하지 않았습니다.
...
다음은 reranker의 품질을 유지하면서 CPU 지연을 줄일 방법을 비교하는 작업입니다.

RRF는 순위표만 보지만 reranker는 `질문 + chunk 본문`을 한꺼번에 읽습니다.
따라서 자연스러운 질문에서 RRF 4위로 내려간 지원대상 chunk를 다시 올릴
가능성이 있습니다. dev 평가에서는 RRF Hit@1 0.60이 reranker 적용 후 0.80으로
올랐지만 평균 지연은 약 6.28초였습니다.
